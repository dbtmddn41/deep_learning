...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        pos_cap_em = self.positional_encoding(self.embedding(captions)) #(N, T)->(N, T, W)
        features_proj = self.visual_projection(features).reshape(N,1,-1) #(N, D)->(N, 1, W)
        tgt_mask = torch.ones((T,T)).tril()

        Y = self.transformer(pos_cap_em, features_proj, tgt_mask=tgt_mask)
