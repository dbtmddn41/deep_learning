...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, next_h = cache #(N, D), (N, H), (D, H), (H, H), (N, H)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      h_prev, cache[t] = rnn_step_forward(x[:, t, :], h_prev, Wx, Wh, b)
      h[:, t, :] = h_prev
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
      dx[:, t, :], dprev_h, dWx_t, dWh_t, db_t = rnn_step_backward(dh_copy[:, t, :], cache[t])
      dWx += dWx_t
      dWh += dWh_t
      db += db_t
      if t != 0:
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, prev_h, prev_c, Wx, Wh, input_gate, forget_gate, output_gate, gate_gate, tanh_next_c)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, prev_h, prev_c, Wx, Wh, i, f, o, g, tanh_next_c = cache

    dhdc, do = dnext_h * o * (1 - tanh_next_c**2), dnext_h * tanh_next_c  #(N, H)
    dc = dnext_c + dhdc   #(N, H)
    df, dprev_c, di, dg = dc * prev_c, dc * f, dc * g, dc * i   #(N, H)
    da = np.concatenate((di * i*(1-i), df * f*(1-f), do * o*(1-o), dg * (1-g**2)), axis=1)    #(N, 4H)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
      h[:, t, :], c,cch = lstm_step_forward(x[:, t, :], prev_h, c, Wx, Wh, b)
      prev_h = h[:, t, :]
      cache.append(cch)
...
def lstm_backward(dh, cache):
    ...
    dh_cp = dh.copy()
    for t in range(T-1, -1, -1):
      dx_t, dh_t, dc, dWx_t, dWh_t, db_t = lstm_step_backward(dh_cp[:,t,:], dc, cache[t])
      dx[:, t, :] = dx_t
      dWx += dWx_t
      dWh += dWh_t
      db += db_t
