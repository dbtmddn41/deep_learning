...
class TestDataset(torch.utils.data.Dataset):
    def __init__(self, audio_paths, sample_rate):
    ...
    def __getitem__(self, idx):
        ...
        (audio, fs) = librosa.core.load(audio_path, sr=self.sample_rate, mono=True)

        data_dict = {
            'audio_name': audio_path, 'waveform': audio,}
            
        audio = torch.tensor(audio)
        audio = pad_truncate_sequence(audio, self.sample_rate*30)
        return audio
