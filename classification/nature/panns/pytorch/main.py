...
from data_generator import GtzanDataset, TrainSampler, EvaluateSampler, collate_fn, TestDataset
...
def inference(args):
    ...
    pretrain = True if finetuned_checkpoint_path else False
    
    if 'cuda' in device:
        print('Using GPU.')
    else:
        print('Using CPU. Set --cuda flag to use GPU.')
    ...
    if pretrain:
        print('Load pretrained model from {}'.format(finetuned_checkpoint_path))
        model.load_state_dict(torch.load(finetuned_checkpoint_path)['model'])
    ...
    dataset = TestDataset(submission['audio_paths'], sample_rate)
    ...
    for audio in test_loader:
        ...
        with torch.no_grad():
            batch_output_dict = model(audio, None)
            """{'clipwise_output': (batch_size, classes_num), ...}"""
            print(batch_output_dict['clipwise_output'].shape)
            _, preds = torch.max(batch_output_dict['clipwise_output'], 1)
        predictions.extend(list(preds.detach().cpu()))
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--submission', type=int, required=True)
    parser_train.add_argument('--finetuned_checkpoint_path', type=int, required=True)
