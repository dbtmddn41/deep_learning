...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          if self.normalization == "batchnorm":
            out, cache[i+1] = affine_batchnorm_relu_forward(out, self.params[f'W{i+1}'], self.params[f'b{i+1}']\
                              , self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'], self.bn_params[i])
          elif self.normalization == "layernorm":
            out, cache[i+1] = affine_layernorm_relu_forward(out, self.params[f'W{i+1}'], self.params[f'b{i+1}']\
                              , self.params[f'gamma{i+1}'], self.params[f'beta{i+1}'], self.bn_params[i])
          else:
            out, cache[i+1] = affine_relu_forward(out, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
          if self.use_dropout:
