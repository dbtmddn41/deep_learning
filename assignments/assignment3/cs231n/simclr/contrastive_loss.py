...
def sim(z_i, z_j):
    ...
    dinominator = torch.linalg.norm(z_j, ord=2) * torch.linalg.norm(z_i, ord=2)
    norm_dot_product = torch.matmul(z_i, z_j.t()) / dinominator
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        molecule = torch.exp(sim(z_k, z_k_N) / tau)
        dinominator = sum(map(lambda x: torch.exp(sim(z_k, x) / tau), out)) - torch.exp(torch.ones(1) / tau)
        # for l in range(2*N):
        #     if l == k:
        #       continue
        #     dinominator += torch.exp(sim(z_k, out[l]) / tau)
        total_loss -= torch.log(molecule / dinominator)

        dinominator = sum(map(lambda x: torch.exp(sim(z_k_N, x) / tau), out)) - torch.exp(torch.ones(1) / tau)
        # for l in range(2*N):
        #     if l == k+N:
        #       continue
        #     dinominator += torch.exp(sim(z_k_N, out[l]) / tau)
        total_loss -= torch.log(molecule / dinominator)
...
def sim_positive_pairs(out_left, out_right):
    ...
    denominator = torch.linalg.norm(out_left, ord=2, dim=1, keepdim=True) * torch.linalg.norm(out_right, ord=2, dim=1, keepdim=True)
    pos_pairs = torch.sum(out_left * out_right, dim=1, keepdim=True) / denominator
...
def compute_sim_matrix(out):
    ...
    out_normed = out / torch.linalg.norm(out, ord=2, dim=1, keepdim=True)
    sim_matrix = torch.matmul(out_normed, out_normed.t())
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    molecule = exponential[torch.arange(N), torch.arange(N-1,2*N-1)][:,None]
    molecule = torch.cat((molecule, molecule), dim=0)
    ...
    numerator = -torch.log(molecule / denom)
